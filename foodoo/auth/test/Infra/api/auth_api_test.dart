...
//type 'ftest' and intellisense will suggest a whole code setup to start with

void main() {
  MockClient client;//dependency
  AuthApi sut;

  setUp(() {
    client = MockClient();//instantiation of client
    sut = AuthApi('http:baseUrl', client);//base url and mock client
  });
  ...
  group('signin', () {
    ...
    test('should return token string when successful', () async {
      ...
      when(client.post(any,
              body: anyNamed('body'), headers: anyNamed('headers')))
          .thenAnswer((_) async =>
          //returns token string instead of malformed token
